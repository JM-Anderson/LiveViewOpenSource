...
Stream_ENVICamera::Stream_ENVICamera(int frWidth,
                       int frHeight,
                       int dataHeight,
                       QObject *parent) :
...
Stream_ENVICamera::~Stream_ENVICamera()
{
    running.store(false);
    is_reading = false;
...
void Stream_ENVICamera::setDir(const char *filename)
{
    // Connect to server

    // Old code to read from file
    qDebug() << filename;
    ...
    readLoopFuture = QtConcurrent::run(this, &Stream_ENVICamera::readLoop);
}

bool Stream_ENVICamera::readHeader(std::string hdr_fname)
{
...
void Stream_ENVICamera::readLoop()
{
    QTime remTime;
...
uint16_t* Stream_ENVICamera::getFrame()
{
